...
### Sync / Async
Note that the blocking API (as presented in the `Getting Started` section) can be used in both, a synchronous and an asynchronous environment. Usually this should be the preferred style of using `httpmock` because it keeps tests simple and you don't need to change the style of usage when switching from a synchronous to an asynchronous environment or vice versa. If you absolutely need to schedule awaiting operations manually, then there are `async` counterparts for every potentially blocking operation that you can use (e.g.: `MockServer::start_async().await`, or `Mock::new().create_on_async(&mock_server)`). 
